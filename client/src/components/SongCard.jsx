...
const SongCard = ({ data, index, type }) => {
    ...
    const [{ allArtists, allAlbums, allSongs, isSongPlaying, songIndex, user }, dispatch] = useStateValue();
    ...
    return (
        <motion.div id="songCard"className="relative w-40 min-w-210 px-2 py-4 cursor-pointer hover:bg-card bg-gray-100 shadow-md rounded-lg flex flex-col items-center" onClick={type === "song" && addtoContext}>
            ...
            <div className="w-full absolute bottom-2 right-2 flex items-center justify-between px-4">
                <motion.i whileTap={{ scale: 0.75 }} className="text-base text-red-400 drop-shadow-md hover:text-red-600" onClick={() => setisDelete(true)}>
                    {location.pathname === "/dashboard/songs" ? <IoTrash /> : null}
                </motion.i>
            </div>
